    ...
    <>
      {parts.map((part, i) =>
        part.toLowerCase() === term.trim().toLowerCase() ? (
          <mark key={i} className="rounded bg-accent/35 px-0.5 text-primary">
            {part}
          </mark>
        ) : (
          part
        ...
        <div className="flex flex-wrap items-start justify-between gap-2">
          ...
          {company && (
            <Link
              ...
              className="shrink-0 text-sm text-muted-foreground transition-colors hover:text-primary hover:underline"
            >
              ← {company.name}
            </Link>
          )}
